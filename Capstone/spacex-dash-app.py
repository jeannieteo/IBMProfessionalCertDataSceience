...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                dcc.Dropdown(id='site-dropdown',value="All", placeholder="Select a Launch Site here", searchable=True,
...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        names='Launch Site', 
        title="Launch Success Launches for Site {}".format(entered_site))
    else:
        filtered_df = spacex_df[spacex_df['Launch Site']==entered_site]
        filtered_df = pd.DataFrame(filtered_df['class'].value_counts()).reset_index() 
        filtered_df.columns = ['class', 'Total']
        fig = px.pie(filtered_df, values='Total',
        names='class',
        title="Launch Success Launches for Site {}".format(entered_site))
    return fig
# TASK 4:
...
def get_scatter_charts(entered_site, entered_payload):
    ...
    if entered_site == 'ALL':
        ...
        #filtered_df = filtered_df.groupby('Booster Version Category',as_index=False)
